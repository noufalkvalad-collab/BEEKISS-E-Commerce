...
export default function AdminOrdersPage() {
    ...
    const filteredOrders = orders.filter(order => {
        const orderIdMask = order._id.slice(-6).toUpperCase();
        const matchesSearch = orderIdMask.includes(searchTerm.toUpperCase()) ||
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={7} className="p-10 text-center">
                                        <div className="flex justify-center items-center py-8">
                                            <Loader2 className="w-8 h-8 animate-spin text-honey-gold" />
                                        </div>
                                    </td>
                                </tr>
                                ...
                                filteredOrders.map((order) => (
                                    <tr key={order._id} className="hover:bg-gray-50/50 transition-colors border-b border-gray-50 last:border-0 group">
                                        <td className="p-4 font-medium text-forest-green font-mono text-xs uppercase">
                                            ORD-{order._id.slice(-6)}
                                        </td>
                                        <td className="p-4">
                                            <div className="font-medium text-gray-900">{order.address?.name || "Guest User"}</div>
                      ...
                      `}>
                                        ...
                                        <td className="p-4 font-semibold text-gray-900">₹{order.totalAmount?.toLocaleString('en-IN') || 0}</td>
